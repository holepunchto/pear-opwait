...
test('match - opwait rejection', async function (t) {
  t.plan(1)
  const stream = new Readable()
  setImmediate(() => {
    stream.destroy(new Error('test'))
  })
  await t.exception(opwait(stream, { tag: 'middle' }), /test/)
})

test('match - opwait operation failure', async function (t) {
  t.plan(2)
  const stream = new Readable()
  setImmediate(() => {
    stream.push({ tag: 'error', data: { stack: 'test', code: 'CHECK' } })
  })
  const op = opwait(stream, { tag: 'middle' })
